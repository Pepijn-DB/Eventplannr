...
import { userValidator, eventValidator, invitationValidator, ifMatchValidator } from '../../src/validators/requestValidator.js';
...
describe('requestValidator', () => {
	...
	it('eventValidator throws when missing or invalid id', () => {
		...
		expect(() => eventValidator({ params: { event_id: 'abc' } } as any)).toThrowError(AppError);
	});
	...
	it('invitationValidator throws when missing or invalid id', () => {
		expect(() => invitationValidator({ params: {} } as any)).toThrowError(AppError);
		expect(() => invitationValidator({ params: { invitation_id: 'x' } } as any)).toThrowError(AppError);
	});

	it('invitationValidator returns number when valid', () => {
		expect(invitationValidator({ params: { invitation_id: '7' } } as any)).toBe(7);
	});
	...
	it('ifMatchValidator throws Precondition failed when etag mismatch', async () => {
		...
		const req = { get: () => 'different', headers: { 'if-match': 'different' } } as any;
		try {
	...
	it('ifMatchValidator returns true when etag matches', async () => {
		...
		const req = { get: () => 'the-etag', headers: { 'if-match': 'the-etag' } } as any;
