...
export async function query(query: string, params: StrNum[] = [], executioner: number):Promise<{ rows: any[] }> {
    switch (dbConfig.type) {
        case 'mysql': {
            if (!await mysql.connect()) {throw new AppError("Database connection error", 500);}
        ...
        case 'postgres': {
            if (!await postgres.connect()) {throw new AppError("Database connection error", 500);}
 ...
 */
export function convertQuestionMarksToDollarParams(sql: string): { sql: string, paramCount: number } {
    let idx = 0;
    let out = '';
