...
import { hasEventPermission, hasLocationPermission } from "../../services/permissionService.js";
import {
    eventValidator,
    userValidator,
...
export const createDateResponse = async (
    ...
    try {
        ...
        const dateId = variableValidator(req.params.date_id) ? Number(req.params.date_id) : null;
        if (!eventId || !dateId) {
        ...
        const resultInvitation = await database.query(sqlInvitation, [userId, eventId], userId);
        if (!resultInvitation || !resultInvitation.rows[0] || !resultInvitation.rows[0].id) {
        ...
        const result = await database.query(sql, [invitationId, dateId, state], userId);
        if (!result) {}
    } catch (err) {
        next(err);
    }
...
export const updateDateResponse = async (
    ...
    try {
        ...
        const result = await database.query(sql, [state, invitationId, dateId], userId);
        if (!result) {
            return res.status(500).json({ message: "Internal server error" });
        }
        return res.status(200).json(result.rows);
...
export const deleteDateResponse = async (
    ...
    try {
        ...
        if (resultInvitation.rows.length === 0 && !(await hasEventPermission(userId, eventId, Event.EDIT_INVITATION))) {
            return res.status(403).json({ message: "Forbidden" });
        }
        const invitationId = resultInvitation.rows[0].id;
...
export const createLocationResponse = async (
    ...
    try {
        ...
        const result = await database.query(sql, [invitationId, locationId, state], userId);
        if (!result) {
            return res.status(500).json({ message: "Internal server error" });
        }
        return res.status(200).json(result.rows);
...
export const deleteLocationResponse = async (
    ...
    try {
        ...
        const result = await database.query(sql, [invitationId, locationId], userId);
        if (!result) {
            return res.status(500).json({ message: "Internal server error" });
        }
        return res.status(200).json(result.rows);
...
export const updateLocationResponse = async (
    ...
    try {
        ...
        const result = await database.query(sql, [state, invitationId, locationId], userId);
        if (!result) {
            return res.status(500).json({ message: "Internal server error" });
        }
    } catch (err) {
