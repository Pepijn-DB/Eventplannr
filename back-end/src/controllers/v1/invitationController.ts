...
import {userValidator, eventValidator, invitationValidator} from "../../validators/requestValidator.js";
...
async function hasEventEditPermission(eventId: number, userId: number): Promise<boolean> {
    const sqlInvitations = `
       SELECT i.user_id, i.event_id, i.role
       FROM invitation i
...
export const getUserInvitations = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const userId = Number(variableValidator(req.params.id) ? req.params.id : userValidator(req));
        const sql = `
            SELECT i.user_id, i.event_id, i.role
            FROM invitation i
            WHERE i.user_id = ?
        `;
        const result = await databaseService.query(sql, [userId], userValidator(req));
        if (!result) throw new AppError("Internal server error");
        return res.status(200).json(result.rows);
    } catch (err) {
        next(err);
...
export const deleteInvitation = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const resultEvent = await databaseService.query(sqlEvent, [invitationId], userId);
...
export const createInvitation = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const invitedUserId = variableValidator(req.body.userId) ? req.body.userId : null;
        ...
        const sql = `
            ...
            VALUES (?, ?, ?)
        `;
        const result = await databaseService.query(sql, [eventId, invitedUserId, role], userId);
        if (!result) throw new AppError("Internal server error");
        return res.status(200).json(result.rows);
    } catch (err) {
        next(err);
...
export const updateInvitation = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (!req.body.role) return res.status(400).json({message: "Nothing to update"});
...
export const getInvitation = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const userId = userValidator(req);
        const eventId = eventValidator(req);
